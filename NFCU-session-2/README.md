...
```bash
make install          # set up the Python 3.12 venv (one time)
make validate         # run everything that does NOT touch AWS — exits 0 when green
```

That is all you need to build and check the lab locally. Provisioning it on AWS
is a separate, opt-in step — see [section B](#b-provisioning-the-lab-on-aws-terraform).
Nothing creates cloud resources until you run a `provision-attendee.sh`.
...
Contributors can also wire the same checks into git via
`.venv/bin/pre-commit install` (config in `.pre-commit-config.yaml`).
...
3. Drive shadow traffic, then evaluate/promote:
   ...
   ```bash
   scripts/trigger-traffic.sh <attendee-id> [duration_minutes] [rate]   # defaults: 5 min, 10 rps
